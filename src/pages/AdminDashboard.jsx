...
import {
    ...
    limit, serverTimestamp, onSnapshot, addDoc
...
import {
    ...
    Globe, BookOpen, User, MoreHorizontal, Check, MessageSquare,
    Trash2, Eye, EyeOff, PieChart, TrendingUp, Star,
    Settings, Bell, AlertTriangle, Send, Archive, X, Info
} from 'lucide-react'
...
export default function AdminDashboard() {
    ...
    const stats = useMemo(() => {
        ...
        const uniqueUsers = new Set(applications.map(a => a.user_email || a.email).filter(Boolean)).size
        const flightBookings = applications.filter(a => a.program_type === 'flight_booking').length
        const visaApps = applications.filter(a => ['study', 'student_visa', 'tourist', 'work', 'residency'].includes(a.program_type)).length
        return { total, pending, approved, processing, rejected, unread, uniqueUsers, flightBookings, visaApps }
    ...
    const updateStatus = async (id, newStatus) => {
        ...
        try {
            ...
            if (statusNote.trim()) {
                updateData.admin_note = statusNote.trim()
            }
    ...
    const markMessageRead = async (id, read = true) => {
        try {
            await updateDoc(doc(db, 'contacts', id), { read, read_at: serverTimestamp(), read_by: currentUser?.email })
        } catch (err) {
                                            ...
                                            recentActivity.map((item, i) => (
                                                <div key={i} className="activity-item">
                                                    <div className={`activity-icon ${item.type === 'message' ? 'msg' : item.app?.status === 'pending' ? 'new' : 'updated'}`}>
                                                        {item.type === 'message' ? <Mail size={14} /> :
                                                         item.app?.status === 'pending' ? <Star size={14} /> :
                                                         <RefreshCw size={14} />}
                                                    </div>
                                                    <div className="activity-content">
                                                        {item.type === 'message' ? (
                                                            ...
                                                            <>
                                                                <strong>{item.app?.full_name || 'Someone'}</strong>{' '}
                                                                {item.app?.status === 'pending' ? 'submitted' : 'updated'} an application
                                                                {item.app?.program_type && <> for <em>{item.app.program_type.replace('_', ' ')}</em></>}
                                                            </>
                                                        )}
                                        ...
                                        {[
                                            ...
                                            return (
                                                <div key={s.label} className="status-bar-row">
                                                    <div className="sbr-label">
                                                        <span className="sbr-dot" style={{ background: s.color }} />
                                                        <span>{s.label}</span>
                                                    </div>
                                                    <div className="sbr-bar-track">
                                                        <div className="sbr-bar-fill" style={{ width: `${pct}%`, background: s.color }} />
                                                    </div>
                        ...
                        <>
                            <div className="admin-filter-bar">
                                ...
                                <div className="filters-group">
                                    ...
                                    <button onClick={exportCSV} className="filter-btn" title="Export CSV">
                                        <Download size={18} />
                                    </button>
                                </div>
                            </div>
                                                        ...
                                                        <tr key={a.id} className={updating === a.id ? 'updating-row' : ''}>
                                                            <td>
                                                                <div className="user-cell">
                                                                    ...
                                                                    <div className="u-info">
                                                                        <span className="n">{a.full_name || 'Unknown'}</span>
                                                                        <span className="e">{a.nationality && `${a.nationality}`}</span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td className="actions-col">
                                                                <div className="admin-action-btns">
                                                                    <button onClick={() => setSelectedApp(a)} className="btn-act view" title="View Details">
                                                                        <Eye size={16} />
                                                                    </button>
                                                                    {a.status !== 'approved' && (
                                                                        <button onClick={() => { setShowNoteInput(a.id); updateStatus(a.id, 'approved') }} className="btn-act app" title="Approve">
                                                                            <Check size={16} />
                                                                        </button>
                                                                    )}
                                                                    {a.status !== 'processing' && (
                                                                        <button onClick={() => { setShowNoteInput(a.id); updateStatus(a.id, 'processing') }} className="btn-act pro" title="Process">
                                                                            <RefreshCw size={16} />
                                                                        </button>
                                                                    )}
                                                                    {a.status !== 'rejected' && (
                                                                        <button onClick={() => { setShowNoteInput(a.id); updateStatus(a.id, 'rejected') }} className="btn-act rej" title="Reject">
                                                                            <XCircle size={16} />
                                                                        </button>
                                                                    )}
                                                                    <button onClick={() => setDeleteConfirm(a.id)} className="btn-act del" title="Delete">
                                                                        <Trash2 size={16} />
                                                                    </button>
                                                                </div>
                                                            </td>
                        ...
                        <div className="admin-split-layout">
                                ...
                                {filteredMessages.length === 0 ? (
                                    ...
                                    <div className="msg-list">
                                        {filteredMessages.map(m => (
                                            <div
                                                key={m.id}
                                                className={`msg-item ${!m.read ? 'unread' : ''} ${selectedMsg?.id === m.id ? 'selected' : ''}`}
                                                onClick={() => { setSelectedMsg(m); if (!m.read) markMessageRead(m.id, true) }}
                                            >
                                                <div className="msg-item-avatar">{m.name?.charAt(0) || '?'}</div>
                                                <div className="msg-item-content">
                                                    <div className="msg-item-header">
                                                        <span className="msg-item-name">{m.name || 'Unknown'}</span>
                                                        <span className="msg-item-time">{timeAgo(m.created_at)}</span>
                                                    </div>
                            ...
                            {selectedMsg ? (
                                <div className="admin-table-card msg-detail-panel">
                                    <div className="card-header">
                                        <div className="card-title-group">
                                            <Mail size={20} className="title-icon" />
                                            <h3>Message Details</h3>
                                        </div>
                                        <div className="msg-detail-actions">
                                            {selectedMsg.read ? (
                                                <button onClick={() => markMessageRead(selectedMsg.id, false)} className="filter-btn" title="Mark as unread">
                                                    <EyeOff size={16} />
                                                </button>
                                            ) : null}
                                            <button onClick={() => deleteMessage(selectedMsg.id)} className="filter-btn del" title="Delete message">
                                                <Trash2 size={16} />
                                            </button>
                                            <button onClick={() => setSelectedMsg(null)} className="filter-btn" title="Close">
                                                <X size={16} />
                                            </button>
                                        </div>
                                    </div>
                                    <div className="msg-detail-body">
                                        <div className="msg-detail-header">
                                            <h2>{selectedMsg.name || 'Unknown'}</h2>
                                            <span className="msg-detail-subject">{selectedMsg.subject || 'General Inquiry'}</span>
                                        </div>
                                        ...
                                        <div className="msg-detail-reply">
                                            <a href={`mailto:${selectedMsg.email}?subject=Re: ${selectedMsg.subject || 'Your Inquiry'}`} className="admin-btn-primary">
                                ...
                                <>
                                    <div className="admin-table-overflow">
                                        <table className="admin-table">
                                            ...
                                            <tbody>
                                                {usersList.map(u => {
                                                    const userApps = applications.filter(a => (a.user_email || a.email) === u.email)
                                                    const latestStatus = userApps.sort((a, b) => {
                                                    ...
                                                    return (
                                                        <tr key={u.email}>
                                                            <td>
                                                                <div className="user-cell">
                                                                    <div className="u-avatar">{u.name?.charAt(0) || 'U'}</div>
                                                                    <div className="u-info">
                                                            ...
                                                            <td>
                                                                {latestStatus ? (
                                                                    <div className={`status-pill-admin ${statusConfig[latestStatus]?.class || 'st-pending'}`}>
                                                                        {statusConfig[latestStatus]?.icon}
                                                                        <span>{statusConfig[latestStatus]?.label || latestStatus}</span>
                                                                    </div>
                                    ...
                                    <div className="admin-table-footer">
                                        <span>{usersList.length} unique users</span>
                                    </div>
                                </>
                            )}
                        </div>
                    ...
                    {activeTab === 'settings' && (
                        <div className="admin-settings-grid">
                            <div className="admin-table-card">
                                <div className="card-header">
                                    <div className="card-title-group">
                                        <Shield size={20} className="title-icon" />
                                        <h3>Admin Access</h3>
                                    </div>
                                </div>
                                <div className="settings-body">
                                    <div className="setting-item">
                                        <div className="setting-info">
                                            <span className="setting-label">Authorized Admin Emails</span>
                                            <span className="setting-desc">These emails can access the admin panel</span>
                                        </div>
                                    ...
                                    <div className="setting-item">
                                        <div className="setting-info">
                                            ...
                                            <span className="setting-desc">You are logged in as an administrator</span>
                                        </div>
                                        <div className="setting-value">
                                            <span className="current-user-badge">{currentUser?.email}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="settings-body">
                                    <div className="setting-item">
                                        <div className="setting-info">
                                            ...
                                            <span className="setting-desc">Data updates automatically via Firestore listeners</span>
                                        </div>
                                        <div className="setting-value"><span className="status-badge-live">Live</span></div>
                                    </div>
                                    <div className="setting-item">
                                        <div className="setting-info">
                                            <span className="setting-label">Total Documents</span>
                                            <span className="setting-desc">Applications + Messages</span>
                                        </div>
                                        <div className="setting-value"><strong>{applications.length + contacts.length}</strong></div>
                                    </div>
                                    <div className="setting-item">
                                        <div className="setting-info">
                                            <span className="setting-label">Data Collections</span>
                                            <span className="setting-desc">Firestore collections in use</span>
                                        </div>
                                        <div className="setting-value">
                                            <div className="admin-emails-list">
                                                <span className="admin-email-chip">applications</span>
                                                <span className="admin-email-chip">contacts</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="admin-modal-body">
                            ...
                            <div className="detail-section">
                                ...
                                <div className="detail-grid">
                                    ...
                                    <div className="detail-item"><label>Status</label>
                                        <div className={`status-pill-admin ${statusConfig[selectedApp.status]?.class || 'st-pending'}`}>
                                            {statusConfig[selectedApp.status]?.icon}
                                            <span>{statusConfig[selectedApp.status]?.label || selectedApp.status}</span>
                                        </div>
                                    </div>
                            ...
                            {selectedApp.message && (
                                <div className="detail-section">
                                    ...
                                    <div className="detail-message-box"><p>{selectedApp.message}</p></div>
                                </div>
                            )}
                        ...
                        <div className="admin-modal-footer">
                            ...
                            <div className="modal-actions-right">
                                ...
                                {selectedApp.status !== 'processing' && (
                                    <button onClick={() => updateStatus(selectedApp.id, 'processing')} className="admin-btn-primary" disabled={updating === selectedApp.id}>
                                        <RefreshCw size={16} /> Mark Processing
                                    </button>
                                )}
                                {selectedApp.status !== 'approved' && (
                                    <button onClick={() => updateStatus(selectedApp.id, 'approved')} className="admin-btn-success" disabled={updating === selectedApp.id}>
